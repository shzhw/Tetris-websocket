define(['config', 'SquareFactory'], function(config, SquareFactory) {
  return function() {
    var gameDiv;
    var nextDiv;
    var timeDiv;
    ...
    var gameData = [];
    // 初始化 data
    for (var i = 0; i < config.gameHeight / config.squareWidth; i++) {
      gameData.push([]);
      for (var j = 0; j < config.gameWidth / config.squareWidth; j++) {
        gameData[i].push(0);
      }
    }
    ...
    function initDiv(container, data, divs) {
      ...
      for (var r = 0; r < data.length; r++) {
        ...
        for (var c = 0; c < data[0].length; c++) {
          ...
          div.className = 'none';
          div.style.top = r * config.squareWidth + 'px';
          div.style.left = c * config.squareWidth + 'px';
